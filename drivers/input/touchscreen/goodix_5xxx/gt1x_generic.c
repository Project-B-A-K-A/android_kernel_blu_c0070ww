...
{
#if defined(ZCFG_TP_GT5XX_KEY_MAP)
		u32 key_map_data[12]=ZCFG_TP_GT5XX_KEY_MAP;
...
void Gt5xx_virtual_keys_destroy(void)
{
...
s32 gt1x_touch_event_handler(u8 * data, struct input_dev * dev, struct input_dev * pen_dev)
{
	...
	GTP_DEBUG(" GTP gt1x_touch_event_handler touch_num: %d\n", touch_num);
 ...
 */
	...
	if ((1==key_value) || (2==key_value) || (4==key_value) || (8==key_value))
	{
		...
		if (KEY_APPSELECT == gt1x_touch_key_array[index_value])
		{
			x = 40;
		}
		else if (KEY_HOMEPAGE == gt1x_touch_key_array[index_value])
		{
			x = 120;
		}
		else if (KEY_BACK == gt1x_touch_key_array[index_value])
		{
			x = 200;
		}
		else if (KEY_SEARCH == gt1x_touch_key_array[index_value])
		{
			x = 280;
		}
...
/* start handle current event and pre-event */
	...
	if (CHK_BIT(cur_event, BIT_TOUCH_KEY) || CHK_BIT(pre_event, BIT_TOUCH_KEY)) {
		...
		input_report_abs(dev, ABS_MT_POSITION_Y, 1380);
		input_report_key(dev, BTN_TOUCH, 1);
		input_mt_sync(dev);
		input_sync(dev);

		if (CHK_BIT(cur_event, BIT_TOUCH_KEY)) {
			GTP_DEBUG("gt1x_touch_event_handler: Key Down.");
		} else {
			GTP_DEBUG("gt1x_touch_event_handler: Key Up.");
			SET_BIT(pre_event, BIT_TOUCH);
			gt1x_touch_up(0);
		}
	#else
...
int Gt5xx_remove_ctp_chip(void)
{
	ctp_lock_mutex();
	tp_device_id(0xFFFF);
	ctp_unlock_mutex();
...
s32 gt1x_init(void)
{
    ...
    ret = check_ctp_chip();
	if (ret < 0) {
		GTP_ERROR("check chip failed(%d): failed to check_ctp_chip\n", ret);
...
/* init auxiliary  node and functions */
	...
	if(device_create_file(tp_cmd_dev, &dev_attr_tp_version) < 0) // /sys/class/mtk-tpd/device/version
	{
