...
#define LSM6DS3_WHO_AM_I_DEF			0x6a
							...
							s32 *xyz, int64_t timestamp)
{
	...
	input_event(input, INPUT_EVENT_TYPE, INPUT_EVENT_X,xyz[1]);
	input_event(input, INPUT_EVENT_TYPE, INPUT_EVENT_Y, -xyz[0]);
...
static DEVICE_ATTR(polling_rate, 0664, get_polling_rate,
							set_polling_rate);
static DEVICE_ATTR(reset_steps, 0664, NULL, reset_steps);
static DEVICE_ATTR(max_delivery_rate, 0664, get_max_delivery_rate,
							set_max_delivery_rate);
static DEVICE_ATTR(sampling_freq_avail, 0664, get_sampling_frequency_avail,
							NULL);
static DEVICE_ATTR(scale_avail, 0664, get_scale_avail, NULL);
static DEVICE_ATTR(scale, 0664, get_cur_scale, set_cur_scale);
