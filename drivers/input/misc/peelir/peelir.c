		...
		struct spi_ioc_transfer *u_xfers)
{
	...
	status = peelir_write(peelir, u_xfers->len);
	return status;
}
...
peelir_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	#if 1///1.5M SPI clk
		id->u32ID1 = 0x2d1a4100;
		id->u32ID2 = 0x37503d40;
		id->u32ID3 = 0x22500000;//0x2e100000;
	#else
...
int peelir_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	peelir = (struct peelir_data *)filp->private_data;
...
static int peelir_probe(struct spi_device *spi)
{
	...
	in_use = 0;
		...
		#if 1
			bpw = 32;
			spi->max_speed_hz=1520 * 1000;///960 * 1000
			mode = 0;
			is_gpio_used = 0;
			field=2345;
			lr_en = 53;
			...
			spi->mode = (u8)mode;
		#else
			of_property_read_u32(np, "peel_ir,spi-bpw", &bpw);
			...
			printk("%s: lr-gpio-valid = %d max_speed_hz=%d\n", __func__, is_gpio_used,spi->max_speed_hz);
			spi->bits_per_word = (u8)bpw;
			spi->mode = (u8)mode;
		#endif
	#else
...
static const struct of_device_id peel_of_match[] = {
	{.compatible = "peel,peel_ir",},
        {}
};
MODULE_DEVICE_TABLE(spi, peel_id);
#endif
