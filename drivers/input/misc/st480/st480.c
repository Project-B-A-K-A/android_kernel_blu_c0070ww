...
static inline s64 st480_get_time_ns(void)
{
	struct timespec ts;
...
{
	s64 timestamp = st480_get_time_ns();

	SENODIAFUNC("st480_input_func");
	st480_work_func();

	// for CTS start
	//if(temp_magx == mag.mag_x )
	//	mag.mag_x += 1;

	//	temp_magx = mag.mag_x;
	// for CTS end
	if (atomic_read(&m_flag) || atomic_read(&mv_flag)) {
		input_event(st480->input_dev, INPUT_EVENT_TYPE, INPUT_EVENT_X, mag.mag_x);
		input_event(st480->input_dev, INPUT_EVENT_TYPE, INPUT_EVENT_Y, mag.mag_y);
		input_event(st480->input_dev, INPUT_EVENT_TYPE, INPUT_EVENT_Z, mag.mag_z);
		input_event(st480->input_dev, INPUT_EVENT_TYPE, INPUT_EVENT_TIME_MSB, timestamp >> 32);
		input_event(st480->input_dev, INPUT_EVENT_TYPE, INPUT_EVENT_TIME_LSB,timestamp & 0xffffffff);
...
int st480_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* Setup input device */
	__set_bit(INPUT_EVENT_TYPE, st480->input_dev->evbit );
	__set_bit(INPUT_EVENT_TIME_MSB, st480->input_dev->mscbit);
	__set_bit(INPUT_EVENT_TIME_LSB, st480->input_dev->mscbit);
	__set_bit(INPUT_EVENT_X, st480->input_dev->mscbit);
	__set_bit(INPUT_EVENT_Y, st480->input_dev->mscbit);
	__set_bit(INPUT_EVENT_Z, st480->input_dev->mscbit);
